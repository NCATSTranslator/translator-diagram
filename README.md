...
## The generated content/ tree

The same inventory as files, with nothing withheld, for people who can read
the repository. This is the demonstration for
[issue #7](https://github.com/NCATSTranslator/translator-diagram/issues/7):
the published page is redacted and public, `content/` is complete and
private, and the page links to it. The design and the runbook for making the
repository private are in
[`docs/public-private-split.md`](docs/public-private-split.md);
[`content/README.md`](content/README.md) explains the files themselves.

```bash
uv run sync-components
uv run build-content --diagram   # -> content/
```

`components.csv` there is the Google Sheet's column layout, generated from
`components/*.yaml`, and `generate-diagram --input content/components.csv`
draws it. A component file may carry a `private:` block; it appears on that
component's page under `content/components/` and nowhere else, and a test
holds that the published payload never contains it. Nothing in `content/`
carries a timestamp, so a rebuild that changes nothing writes nothing; the
manual `Refresh content` workflow opens a pull request with whatever a fresh
sync changed.
...
```text
uv run build-content [OPTIONS]

  --components DIRECTORY           Directory of component YAML files  [default: components]
  --sync-dir DIRECTORY             Cache written by sync-components. Without one,
                                   only the files a checkout alone determines are
                                   written  [default: data/sync]
  --output-dir DIRECTORY           The content tree to write  [default: content]
  --diagram / --no-diagram         Also render diagram.svg from the generated
                                   components.csv. Needs Graphviz  [default: no-diagram]
```

`build-dashboard` also takes `--content-url` (default: this repository's
`content/` on `main`; an empty string drops the link from the page).
...
│   ├── content.py        # The content/ tree: Markdown and CSV with nothing withheld
...
├── content/              # Generated by build-content and committed — see its README
│   ├── components.csv    # The sheet's column layout, from components/*.yaml
│   ├── deployments.csv   # Every field of every cell
│   ├── dashboard.md      # The table, by stage
│   ├── diagram.svg       # Drawn from components.csv
...
├── docs/                 # The metadata case, its research, owner colours, the #7 design
├── tests/                # One test file per module
├── .github/workflows/    # ci.yml (tests and lints), pages.yml (build + deploy),
│                         # content.yml (refresh content/ through a pull request)
...
contains. The proposed answer — a private repository, a public page pushed
to a separate site repository, and a committed `content/` tree for everyone
with access — is built as a demonstration and written up in
[`docs/public-private-split.md`](docs/public-private-split.md). `content/`
is the one generated tree that is committed; `data/` stays gitignored.
