...
function rowHtml(row) {
  ...
  const repo = row.repository
    ? `<a href="${esc(row.repository)}">${esc(row.repository.replace("https://github.com/", ""))}</a>`
    : DASH;
...
function shell() {
  ...
  <div class="wrap">
    ...
    <footer>
      ...
      with no recorded dependencies. Generated by <code>build-dashboard</code>.
    </footer>
  </div>`;
}
